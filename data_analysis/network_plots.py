...
def main_generation(number_nodes):
   ...
   # Plotting the graph
    ...
    workers_plot = nx.draw(
        worker_network_nx, with_labels=False, node_color='blue') #, node_size=[v * 100 for v in worker_network_nx.deg()])
    plt.savefig('workers_network.png', dpi=300)
    ...
    random_plot = nx.draw(
        random_network_nx, with_labels=False, node_color='red')#,  node_size=[v * 100 for v in random_deg.values()])
    plt.savefig('randomsocial_network.png', dpi=300)

    # Plotting the graph
    plt.figure(4)
    #student_deg = student_network_nx.degree()
    student_plot = nx.draw(student_network_nx,
                          with_labels=False, node_color='pink')#,  node_size=[v * 100 for v in student_deg.values()])
    plt.savefig('student_network.png', dpi=300)

    # Plotting the graph
    plt.figure(5)
    essential_deg = essential_network_graph_nx.degree()
    essential_plot = nx.draw(essential_network_graph_nx,
                          with_labels=False, node_color='yellow')#,  node_size=[v * 100 for v in essential_deg.values()])
    plt.savefig('essentialworker_network.png', dpi=300)

    # Composition of the graph together
    composition_graph = nx.compose_all(
        [family_network_nx, worker_network_nx, random_network_nx, student_network_nx]) #essential_network_graph_nx,
    deg_composite = degree_distribution(composition_graph)

    # Ploting the graph
    plt.figure(6)
    #essential_random_deg = essential_random_network_nx.degree()
    essential_random_plot= nx.draw(essential_random_network_nx, with_labels=False, node_color='black')#,  node_size=[v * 100 for v in essential_random_deg.values()])
    plt.savefig('essentialrandom_network.png', dpi=300)

    # Ploting the graph
    plt.figure(7)
    #composite_deg = composition_graph.degree()
    composite_plot= nx.draw(composition_graph, with_labels=False)#,  node_size=[v * 100 for v in composite_deg.values()])
